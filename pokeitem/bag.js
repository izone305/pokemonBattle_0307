...
function bagSelectWindow() {
  let bagWholeWindow = window.document.body;
    ...
    styleSet(menuWrap, '90%', '10%', '1px solid red');
    ...
    for (let i = 0; i < 4; i++) {
      ...
      styleSet(bagMenu[i], '24%', '70%', '1px solid red');
    }
    ...
    styleSet(itemWrap, '90%', '80%', '1px solid red');
    ...
    // itemList
    let itemObject = {
      healing: ["potion", "super-potion", "max-potion", "fresh-water", "lemonade", "full-restore"],
      pokeball: ["master-ball", "ultra-ball", "great-ball", "poke-ball", "safari-ball", "park-ball"],
      battle: ["guard-spec", "dire-hit", "x-attack", "x-defense", "x-speed", "x-accuracy"],
      berrys: ["cheri-berry", "chesto-berry", "pecha-berry", "rawst-berry", "aspear-berry", "leppa-berry"]
    }
    ...
    function itemDetail(itemNum, num, itemcontent) {
      ...
      itemxhr.addEventListener("load", function () {
        ...
        count = 0;
        while (true) {
          if (objectItem.flavor_text_entries[count].language.name === "ko") { break; };
          count += 1;
          if (count > 20) { break; }
        }
